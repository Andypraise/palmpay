...
function Airtime() {
  ...
  const handleRecharge = () => {
    if (!phone || !amount || !network) return alert("Enter valid phone, network, and amount");
    alert(`Recharging ${phone} with ₦${amount} on ${network}`);
    setPhone("");
    setAmount("");
    setNetwork("");
  };
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-4 flex flex-col items-center">
      <h2 className="text-2xl font-bold mb-6">Airtime Recharge</h2>
      ...
      {network && (
        <div className="flex items-center gap-2 mb-4 font-medium text-gray-700">
          <img src={logos[network]} alt={network} className="w-6 h-6" /> {network} Network detected
        </div>
      )}
      ...
      <button
        ...
        className={`w-full max-w-md py-3 rounded-lg font-semibold text-white transition ${
          phone && amount && network ? "bg-purple-700 hover:bg-purple-800" : "bg-gray-300 cursor-not-allowed"
        }`}
      >
        Recharge
      </button>
